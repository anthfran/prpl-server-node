...
By default, if a `404 Not Found` or other HTTP server error occurs, prpl-server will write a minimal `text/plain` response. To serve custom errors, [use prpl-server as a library](#as-a-library) with Express and register an [error-handling middleware](http://expressjs.com/en/guide/using-middleware.html#middleware.error-handling) after registering the prpl-server handler.
